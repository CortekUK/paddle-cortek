...
create policy org_update on public.organizations
  for update using (
    exists (select 1 from public.profiles p where p.user_id = auth.uid() and p.organization_id = organizations.id and p.role in ('org_admin','super_admin'))
);
...
create policy wr_mutate_admins on public.whatsapp_routes
  for all using (
    exists (select 1 from public.profiles p where p.user_id = auth.uid() and p.organization_id = whatsapp_routes.organization_id and p.role in ('org_admin','super_admin'))
);
...
create policy billing_update_admins on public.billing_stub
  for update using (
    exists (select 1 from public.profiles p where p.user_id = auth.uid() and p.organization_id = billing_stub.organization_id and p.role in ('org_admin','super_admin'))
);
...
begin
  ...
  insert into public.profiles (user_id, organization_id, first_name, last_name, phone, email, role)
