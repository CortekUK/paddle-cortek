...
const timezones = ['Europe/London', 'Europe/Paris', 'Europe/Madrid', 'America/New_York', 'America/Los_Angeles', 'Asia/Tokyo'];
interface Location {
  id: string;
  name: string;
  timezone: string;
    ...
    const fetchLocation = async () => {
      ...
      try {
        const {
          data,
          error
        } = await supabase.from('locations').select('id, name, timezone').eq('id', profile.location_id).single();
  ...
  return <div className="space-y-6">
      ...
      <div className="relative -mx-8 -mt-8 px-8 py-10 bg-gradient-to-r from-primary/20 via-purple-500/15 to-primary/10 dark:from-primary/15 dark:via-purple-500/10 dark:to-primary/8 border-b border-primary/15">
        ...
        <div className="relative text-left">
          ...
          <p className="text-muted-foreground mt-1">Configure your padel club location details.</p>
        </div>
      </div>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6 text-left">
              <div className="space-y-2">
                ...
                <Input id="name" placeholder="e.g., Pure Padel Manchester" value={formData.name} onChange={e => setFormData(prev => ({
                ...prev,
                name: e.target.value
              }))} required disabled={loading} className="h-11 rounded-lg" />
              </div>

              <div className="space-y-2">
                <Label htmlFor="timezone">Timezone</Label>
                <Select value={formData.timezone} onValueChange={value => setFormData(prev => ({
                ...prev,
                timezone: value
              }))} disabled={loading}>
              ...
              <Button type="submit" disabled={loading || !formData.name.trim()} variant="hero" className="w-full rounded-xl">
